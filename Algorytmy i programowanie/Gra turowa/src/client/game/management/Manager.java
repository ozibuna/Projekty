...
public class Manager implements IGameManager {
    /**
     ...
     *
     * @see Manager#nextTurn()
     * @see Manager#currentPlayerTurn
     */
     ...
     */
    public Manager(int totalPlayers) {
...
//        }
    ...
    private boolean isPlayerTurn() {
        return currentPlayerTurn == player.getPlayerID();
    }
    ...
    private boolean checkIfWon(){
        ...
        for (int i = 0; i < totalPlayers; i++){
            ...
            if (i == player.getPlayerID()) continue;
    ...
    public void playTurn() {
        ...
        if (isGameActive && isPlayerTurn()) {
            if (player.getCastle().isDestroyed()) {
                gameOver();
                return;
            } else if (checkIfWon()) {
                gameWon();
            ...
            /*
            for (Hero u: player.getCastle().getTroops().get("Worker")) {
                u.getTile().returnResource(player.getCastle());
            }
             */
            ...
            */
            player.getCastle().updateMovementPoints();
            worldMap.runNPCActions(player.getCastle());
            if (isNetworkGame) {
                canMove = true;
            }
            if (player.getCastle().isDestroyed()) {
    ...
    private void assignCastle(int playerID) {
        player.setCastle(worldMap.getCastles().get(playerID));
    }
